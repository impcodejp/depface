...
pub fn verify_password(password: &str, hash: &str) -> Result<bool, String> {
    let parsed_hash = PasswordHash::new(hash)
        .map_err(|e| format!("ハッシュ形式が不正です: {}", e))?;
...
mod tests {
    ...
    fn test_password_hashing_and_verification() {
        ...
        // 生パスワードが含まれていないこと
        assert!(!hash.contains(password), "ハッシュ値に生パスワードが含まれています");
        
        // Argon2 の標準形式（$argon2id$...）で始まっていること
        assert!(hash.starts_with("$argon2id$"), "ハッシュ形式が Argon2id ではありません: {}", hash);
        
        // 十分な長さ（複雑さ）があること（通常 90文字前後になります）
        assert!(hash.len() > 50, "ハッシュ値が短すぎます。複雑さが足りない可能性があります");
        ...
        // 4. 間違ったパスワードでの検証
        let is_invalid = verify_password("wrong_password", &hash).expect("検証処理自体に失敗しました");
        assert!(!is_invalid, "間違ったパスワードなのに検証をパスしてしまいました");
    }
    ...
    fn test_salt_is_random() {
        ...
        assert_ne!(hash1, hash2, "同じパスワードで同じハッシュ値が生成されました。ソルトがランダムではありません");
    }
}
